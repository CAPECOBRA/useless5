...
class CheckoutView(EcomMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if cart_id:
            ...
            form.instance.payment_status = "Payment Intiated"
            del self.request.session['cart_id']
            pm = form.cleaned_data.get("payment_method")
            order = form.save()
            if pm == "UPI":
                return redirect (reverse("ecomapp:UPI"))
...
class AdminHomeView(AdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["pendingpayments"] = Order.objects.filter(
            payment_status="Payment Initiated").order_by("-id")
        return context
...
class AdminOrderDetailView(AdminRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["allstatus"] = PAYMENT_STATUS
        return context
...
class AdminPaymentStatusChangeView(AdminRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        order_id = self.kwargs["pk"]
        order_obj = Order.objects.get(id=order_id)
        new_Pstatus = request.POST.get("paymentstatus")
        order_obj.payment_status = new_Pstatus
        order_obj.save()
        return redirect(reverse_lazy("ecomapp:adminorderdetail", kwargs={"pk": order_id}))
